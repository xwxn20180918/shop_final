  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item " :class="classColors[support.type]" v-for="(support,index) in info.supports" :key="index">
...
<script>
  ...
  export default {
    data(){
      return{
        classColors:['activity-orange','activity-green','activity-red']
      }
    },
    mounted(){
      this.$store.dispatch('getInfo',() =>{
        if(!this.scroll){
          this.$nextTick(() => {
            ...
            //动态计算ul宽度
            const ul =this.$refs.refUl
            const liWidth = 120
            const spance = 6
            const count =this.info.pics.length
            ul.style.width = (liWidth + spance)* count - spance + 'px'
